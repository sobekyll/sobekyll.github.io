...
**SobeKyll** 是一个基于[SoberJS](https://soberjs.com/)开发的[Jekyll](https://jekyllrb.com/) Material Design风格的主题.

## 安装
### 在Github Pages中使用

> 因为Github Pages对Jekyll以及相关库和版本的支持都有限，默认只支持部分库以及有限的版本，所以在Github Pages中使用Sobekyll需要特别说明。相关信息请参考: [Dependency versions](https://pages.github.com/versions/)

在你博客目录的根目录下，创建`.github/workflows/jekyll.yml`action配置文件，并添加如下代码：
```yml
  ...
  build:
    ...
    - name: Deploy to GitHub Pages
      ...
      with:
        ...
        publish_dir: ./_site
```
> 这里需要注意的是分支名称`main`，在部分早期创建的repo中，需要改为`master`，这取决于你项目的主分支是什么。
```yml
on:
  push:
    branches:
      - master  # 早期项目需要改为master
```

在`Gemfile`中，添加`gem 'sobekyll'`并执行`bundle install`，然后在`_config.yml`文件中配置主题：
```yml
theme: sobekyll
```
设置完成，将修改推送到GitHub，并在repo的**Settings -> Pages**, 将**Build and deployment** 项改为**Deploy from a branch**, 并把分支改为'gh-pages'。修改后需要等待一段时间生效。
